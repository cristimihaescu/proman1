...
async function statusBoardManager(board) {
    ...
    const myBoard = document
        .querySelector(`[data-board-id="${boardId}"]`)
        .closest(".board-container");
    const myStatusContainer = myBoard.querySelector(
        'div[class="status-container"]'
    );
    for (const status of boardStatuses) {
        myStatusContainer.appendChild(createStatusBoxes(status, board.id));
        const addCardLinks = myStatusContainer.querySelectorAll(
            `.new-card-link[data-board-id="${board.id}"]`
        );
        const deleteStatusLinks = myStatusContainer.querySelectorAll(
            `.delete-status[data-board-id="${board.id}"]`
        );
        addCardLinks.forEach((link) =>
            link.addEventListener("click", addNewCard)
        );
        deleteStatusLinks.forEach((link) =>
